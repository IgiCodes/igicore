...
namespace IgiCore.Server.Services
{
	public class VehicleService : ServerService
	{
		...
		private void ReassignTrackedVehicles([FromSource] Citizen disconnectedCitizen, string disconnectMessage, CallbackDelegate kickReason)
		{
			...
			List<Vehicle> vehicles = Db.Vehicles.Where(v => v.TrackingUserId == disconnectedUser.Id).ToList();
			...
			foreach (Vehicle vehicle in vehicles)
			{
				foreach (User user in users)
				{
					foreach (Character character in user.Characters)
					{
						...
						{
							if (Vector3.Distance(character.Position, vehicle.Position) < Vector3.Distance(assignedVehicles[vehicle].Item2.Position, vehicle.Position))
							{
			...
			if (!assignedVehicles.Any())
			{
				...
				try
				{
					hostClient = this.server.Players.First(p => p.Handle == API.GetHostId());
				}
				catch (Exception ex)
				{
					Log(ex.Message);
			...
			{
				Citizen citizen = this.server.Players.First(c => c.Identifiers["steam"] == Db.Users.First(u => u.Id == assignedVehicle.Value.Item1.Id).SteamId);
