...
namespace IgiCore.Server.Extentions
{
	...
	{
		public static IEnumerable<Type> GetTypesInNamespace(this Type type, string nameSpace)
		{
			return type.GetTypeInfo().Assembly.GetTypesInNamespace(nameSpace);
		}
		...
		{
			return type.GetTypesInNamespace(type.Namespace).Where(t => !t.IsAbstract && t.IsPublic && t.IsSubclassOf(type));
		}
	}
}
