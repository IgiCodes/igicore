...
namespace IgiCore.Server
{
    public partial class Server : BaseScript
    {
        ...
        private static Character GetCharCommand(Citizen citizen, Guid charId) { return User.GetOrCreate(citizen).Characters.FirstOrDefault(c => c.Id == charId); }

        private static Character GetCharCommand(Citizen citizen, string name)
        {
            return User.GetOrCreate(citizen)
                .Characters
